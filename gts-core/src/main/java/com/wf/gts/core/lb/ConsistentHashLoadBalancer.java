...
public class ConsistentHashLoadBalancer  {
    ...
    public LiveManageInfo doSelect(String ip,List<LiveManageInfo> liveManageInfos) {
        int hashcode = liveManageInfos.hashCode(); // 判断是否同样的服务列表
        Selector selector = selectorCache.get(ip);
        if (selector == null // 原来没有
            ||
            selector.getHashCode() != hashcode) { // 或者服务列表已经变化
            selector = new Selector(ip, liveManageInfos, hashcode);
            selectorCache.put(ip, selector);
        }
